        ...
        <section class="hero">
            <div class="hero-content">
                ...
                <p class="hero-subtitle">
                    Discover stories that inspire learning beyond the classroom
                </p>
                <v-btn
                    color="white"
                    variant="outlined"
        ...
        <main class="main-content">
            ...
            <section v-if="!selectedPost && featuredPost" class="featured-post">
                ...
                <div class="featured-card" @click="selectPost(featuredPost)">
                    <v-img
                        :src="featuredPost.image_url || defaultImage"
                    ...
                    <div class="featured-content">
                        ...
                        <div class="author-info">
                            <v-avatar size="40" color="primary">
                                <v-img
                                    v-if="featuredPost.author_avatar_url"
            ...
            <section class="newsletter">
                <div class="newsletter-content">
                    ...
                    <h3>Stay Updated</h3>
                    <p>
                        Get the latest educational travel insights delivered to
                        your inbox
                    </p>
    ...
    switch (platform) {
        ...
        case "linkedin":
            shareUrl = `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(
                url
            )}&title=${encodeURIComponent(selectedPost.value.title)}`;
            break;
        case "whatsapp":
...
onMounted(async () => {
    try {
        ...
        const res = await axios.get("/api/blogs", {
            params: { limit: 12, order: "desc" },
        });
        console.log("🟢 Raw server response:", res.data);

        /* locate the array, whatever key it’s under */
        let rawPosts = [];
        if (Array.isArray(res.data)) rawPosts = res.data;
        else if (Array.isArray(res.data.data))
            rawPosts = res.data.data; // Laravel resource
        else if (Array.isArray(res.data.items))
            rawPosts = res.data.items; // “items”
        else if (Array.isArray(res.data.posts))
            rawPosts = res.data.posts; // “posts”
        else {
            console.error("⚠️  Could not find array of posts in:", res.data);
        }

        /* map into the shape your UI needs */
        ...
        if (postId) {
            selectedPost.value =
                posts.value.find((p) => `${p.id}` === postId) ?? null;
        }
    } catch (error) {
